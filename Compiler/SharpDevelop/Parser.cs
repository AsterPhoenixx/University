...
namespace scsc
{
	public class Parser
	{
	   ...
	   // Location = Ident | Ident '[' Expression ']'.
		...
		public bool IsStepStatementNikolai() //1701261009
		{
			Type type;
			if (CheckKeyword("steps"))
			{
				// 'steps' '(' Expression ')' Statement
				...
				if (!CheckSpecialSymbol("(")) Error("Очаквам специален символ '('");
				if (!IsExpression(null, out type)) Error("Очаквам израз");
				if (!AssignableTypes(typeof(System.Int32), type)) Error("Типа на изразът трябва да е Int32");
				if (!CheckSpecialSymbol(")")) Error("Очаквам специален символ ')'");
